    ...
    for r in runbooks:
        ...
        <div style="background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 4px; padding: 12px; margin-bottom: 12px;">
            ...
            <div style="margin-bottom: 6px; color: #334155;">
                <strong>Root Cause:</strong> {r['cause']}
            </div>
            <div style="background-color: #0f172a; color: #f8fafc; font-family: monospace; padding: 10px; border-radius: 4px; font-size: 8.5pt; white-space: pre-wrap;">
                {r['solution']}
            </div>
        </div>
        """
...
st.title("DevOps Knowledge Base")
st.caption(
    "Internal repository for infrastructure runbooks and incident resolution protocols."
)
st.markdown("---")
...
search_query = st.sidebar.text_input(
    "Search (ID, Tag, Keyword):",
    placeholder="e.g. crashloop, ingress, db"
)

# Category Setup
existing_categories = sorted(
    list(set([r["category"] for r in runbooks if "category" in r]))
)
...
all_categories = sorted(
    list(set(existing_categories + default_categories))
)
...
with st.sidebar.form("add_runbook_form", clear_on_submit=True):
    ...
    custom_cat = st.text_input(
        ...
        placeholder="Specify if custom option selected"
    )
    ...
    if submit_btn:
        final_category = (
            custom_cat.strip()
            if selected_cat == "+ Add Custom Category..."
            else selected_cat
        )
        ...
        if new_id and new_title and final_category:
            ...
            st.sidebar.error(
                "ID, Title, and Category fields are required."
            )
...
col_m1.metric(
    "Total Active Runbooks",
    len(runbooks)
)

col_m2.metric(
    "Filtered Results",
    len(filtered_runbooks)
)
...
if not filtered_runbooks:
    st.info(
        "No runbook entries match the current filter criteria."
    )
